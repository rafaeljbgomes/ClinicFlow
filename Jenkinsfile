pipeline {
    ...
    stages {
        ...
        stage('Compose and Playwright') {
            when { expression { params.VALIDATION_LEVEL == 'FULL' } }
            steps { sh 'pwsh -NoLogo -NoProfile -File ./scripts/ci/Invoke-ComposePlaywright.ps1 -RunId $CI_RUN_ID -RuntimeHost host.docker.internal' }
        }
    }
    ...
    post {
        always {
            ...
            script {
                if (fileExists('coverage-report/target/site/jacoco-aggregate/index.html')) {
                    publishHTML(target: [allowMissing: false, alwaysLinkToLastBuild: true, keepAll: true, reportDir: 'coverage-report/target/site/jacoco-aggregate', reportFiles: 'index.html', reportName: 'JaCoCo aggregate'])
                }
                if (fileExists('frontend/playwright-report/index.html')) {
                    publishHTML(target: [allowMissing: false, alwaysLinkToLastBuild: true, keepAll: true, reportDir: 'frontend/playwright-report', reportFiles: 'index.html', reportName: 'Playwright'])
                }
            }
