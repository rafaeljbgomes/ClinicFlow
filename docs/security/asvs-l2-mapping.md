...
| Kubernetes network controls | The local Kubernetes chart declares default-deny NetworkPolicies and then allows only frontend-to-service, service-to-database, service-to-RabbitMQ, and monitoring-to-metrics traffic. | `deploy/helm/clinicflow/templates/networkpolicy.yaml`. |
